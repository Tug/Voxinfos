...
public class ArrayList extends AbstractCollection implements List
{
    ...
    public void serialize(DataOutput output) throws IOException {
        output.writeInt(theSize);
        for(int i=0; i<theSize; i++) {
            Object item = theItems[i];
            if(item instanceof Serializable) {
                Serializable serializableObject = (Serializable) item;
                Serializer.serializeClass(serializableObject, output);
            } else {
    ...
    public void deserialize(DataInput input) throws IOException {
        this.theSize = input.readInt();
        this.theItems = new Object[theSize];
        this.modCount = theSize;
        for(int i=0; i<theSize; i++) {
            theItems[i] = Serializer.deserializeClass(input);
        }
    }
